...
function arcRoundedPath(start, end, radius, outerOldEnd, innerOldEnd, arcStartOne, arcEndOne, arcStartTwo, arcEndTwo, clockWise, innerStart, innerEnd, innerOldStart, outerOldStart, pointPosition) {
    let roundedPath = 'M ' + start.x + ' ' + start.y + ' A ' + radius + ' ' + radius + ' 0 ' +
...
function getRangePath(start, end, innerStart, innerEnd, radius, startRadius, endRadius, arcRadius, clockWise, center, degree, range, axis) {
    let startWidth = range.startWidth;
    let endWidth = range.endWidth;
    let endArc;
    let startArc;
...
function arcWidthPathCalculation(start, end, innerStart, innerEnd, radius, startRadius, endRadius, arcRadius, clockWise, center, outerOldEnd, innerOldEnd, outerOldStart, innerOldStart, startWidth, endWidth, degree, range, axis) {
    if (!isNullOrUndefined(range)) {
        ...
        let arcStartTwo;
        let arcEndTwo;
        let startValueToAngle = getAngleFromValue(((range.start + range.end) / 2), axis.maximum, axis.minimum, axis.startAngle, axis.endAngle, axis.direction === 'ClockWise');
        let pointPosition = (startWidth < ((endWidth))) ?
            getLocationFromAngle(startValueToAngle, endRadius, center) : getLocationFromAngle(startValueToAngle, startRadius, center);
        let endDistance = Math.sqrt((Math.pow((innerEnd.x - pointPosition.x), 2)) + (Math.pow((innerEnd.y - pointPosition.y), 2)));
        let endRadii = endDistance / 2;
        let centerDistance = Math.sqrt((Math.pow((center.x - pointPosition.x), 2)) + (Math.pow((center.y - pointPosition.y), 2)));
        let centerStartDistance = Math.sqrt((Math.pow((center.x - innerStart.x), 2)) + (Math.pow((center.y - innerStart.y), 2)));
        if (range.roundedCornerRadius <= 0) {
            ...
            let degreeValue = range.position === 'Cross' ? 330 : 325;
            if (((degreeValue <= degree && degree <= 360))) {
                ...
                let arcEnd = (arcRadius - (widthDifference / 2));
                let angleValueDirection = axis.direction === 'ClockWise' ? degree >= 310 : degree < 345;
                if (degree < 310) {
                ...
                if (range.position === 'Cross') {
                    let endRadiusValue = axis.direction === 'ClockWise' ? degree <= 300 && degree >= 260 ?
                    ...
                    let startRadiusValue = axis.direction === 'ClockWise' ? degree > 325 ? degree > 340 ? (startRadius - startWidth)
                    ...
                    if (degree < 325 && degree > 285) {
                        let arcOne;
                        let arcTwo;
                        let startGreater = startWidth / 2;
                        let endGreater = endWidth / 2;
            ...
            let degreeAngle = axis.endAngle < 4 ? 356 : 360;
            clockWise = degree > degreeAngle ? 0 : clockWise;
            let degreeValueOne = axis.direction === 'ClockWise' ? 327 : 322;
            let degreeValueTwo = axis.direction === 'ClockWise' ? 328 : 325;
            if ((endWidth === startWidth) && (axis.startAngle !== 0 || axis.endAngle !== 0)) {
                ...
                let startAddArc = endRadius + (widthDifference / 2) - (endWidth / 2);
                let startSubArc = endRadius - (widthDifference / 2) - (endWidth / 2);
...
let CircularGauge = class CircularGauge extends Component {
    ...
    setAnnotationValue(axisIndex, annotationIndex, content) {
        ...
        if (content !== null) {
            ...
            let secondaryElement = getElement(this.element.id + '_Secondary_Element');
